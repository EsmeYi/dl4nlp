"""
codebert_baseline.py — CodeBERT as source encoder baseline.

Two evaluations:
  1. Zero-shot probe: CodeBERT (no fine-tuning) → predict IPC via Ridge regression.
     Answers: does CodeBERT already have hardware intuition?
  2. Fine-tuned retrieval: CodeBERT src_enc + custom asm_enc trained with InfoNCE.
     Compares recall@1 directly against our from-scratch model.

Usage: python codebert_baseline.py
"""
...
CB_MODEL      = "microsoft/codebert-base"
CB_MAX_LEN    = 512
EMBED_DIM     = 256
PROJ_DIM      = 128
NUM_LAYERS    = 4
...
BATCH_SIZE    = 48    # smaller: CodeBERT uses more VRAM
LEARNING_RATE = 1e-4
CB_LR         = 2e-5
WEIGHT_DECAY  = 0.01
TEMPERATURE   = 0.05
...
# ── Dataset with CodeBERT tokenization in src_ids ─────────────────────────────

class CBTripletDataset(Dataset):
    """Like TripletDataset but tokenizes source with CodeBERT tokenizer."""
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        return {
            # src_ids here are CodeBERT token IDs (pad=1 in RoBERTa)
...
class CBSrcEncoder(nn.Module):
    """CodeBERT source encoder. forward(ids) accepts RoBERTa token IDs (pad=1)."""
...
# ── Zero-shot CodeBERT probe ───────────────────────────────────────────────────

@torch.no_grad()
def zero_shot_probe(cb_enc: CBSrcEncoder, cb_train_loader, cb_val_loader, device):
    """Ridge regression on frozen CodeBERT embeddings → predict IPC."""
    ...
    print("  Collecting CodeBERT embeddings (train) ...")
    X_tr, y_tr = collect(cb_train_loader, PROBE_TR_N)
    print("  Collecting CodeBERT embeddings (val) ...")
...
def main():
    ...
    print(f"  CodeBERT params: {sum(p.numel() for p in cb_enc_frozen.parameters())/1e6:.1f}M")
    ...
    # ── 2. Fine-tuned CodeBERT + AsmEncoder retrieval ───────────────────────
    print("\n=== Fine-tuned CodeBERT retrieval ===")
